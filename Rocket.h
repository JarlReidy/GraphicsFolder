...
class Rocket : public GeometryHelper
{
...
public:
    Rocket(int nSubDivisions)
    {
#include "rocketverts.inc"
        glm::vec3 vertexPositions[nRocketVerts];
        for (int iVerts = 0; iVerts < nRocketVerts; iVerts++)
        {
            vertexPositions[iVerts] = glm::vec3(rVerts[3 * iVerts + 0], rVerts[3 * iVerts + 1], rVerts[3 * iVerts + 2]);
        }
        for (int iPatch = 0; iPatch < nRocketPatches; iPatch++)
        {
